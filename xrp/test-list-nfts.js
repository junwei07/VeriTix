...
export async function ListNftsAndOffers(client, account) {
  
  if (!account) throw new Error("Missing env var: MINTED_NFT_ACC");
  if (!xrpl.isValidClassicAddress(account)) {
    throw new Error(`Invalid classic address: "${account}"`);
  }

  console.log("Sending view NFT request...")
  ...
  for (let i = 0; i < nft_keylets.length; i++) {
    ...
    try {
      ...
      console.log(e);
      console.log(
        `Couldn't get buy offers for NFT ${nft_keylets[i]}. Maybe there aren't any.`
      );
      continue;
    }
    ...
    for (let j = 0; j < response_1.result.offers.length; j++) {
      console.log(`\n${j + 1}.`);
      console.log(
        ` NFT Offer Index: ${response_1.result.offers[j].nft_offer_index}`
      );
      console.log(
        `    Offer Amount: ${response_1.result.offers[j].amount} drops`
      );
